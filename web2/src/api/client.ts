...
export default class ApiClient {
    ...
    createUser(request : Model.CreateUserRequest) : Promise<Model.User> {
        const route = `/users`;
    ...
    login(request : Model.LoginRequest) : Promise<Model.Session> {
    ...
    getBoard(regionCount : number) : Promise<Model.Board> {
        const route = `/boards/${regionCount}`;
        return ApiClientCore.sendRequest<{}, Model.Board>(
            HttpMethod.Get, route);
    }

    getCellPaths(regionCount : number, cellId : number) : Promise<number[][]> {
    ...
    addPlayer(gameId : number, request : Model.CreatePlayerRequest) : Promise<Model.StateAndEventResponse> {
        const route = `/games/${gameId}/players`;
        return ApiClientCore.sendRequest<Model.CreatePlayerRequest, Model.StateAndEventResponse>(
            HttpMethod.Post, route, request);
    }

    removePlayer(gameId : number, playerId : number) : Promise<Model.StateAndEventResponse> {
        const route = `/games/${gameId}/players/${playerId}`;
        return ApiClientCore.sendRequest<{}, Model.StateAndEventResponse>(
            HttpMethod.Delete, route);
    }

    updatePlayerStatus(gameId : number, playerId : number, status : Model.PlayerStatus) : Promise<Model.StateAndEventResponse> {
    ...
    commitTurn(gameId : number) : Promise<Model.StateAndEventResponse> {
        const route = `/games/${gameId}/current-turn/commit-request`;
        return ApiClientCore.sendRequest<{}, Model.StateAndEventResponse>(
            HttpMethod.Post, route);
    }

    resetTurn(gameId : number) : Promise<Model.StateAndEventResponse> {
        const route = `/games/${gameId}/current-turn/reset-request`;
        return ApiClientCore.sendRequest<{}, Model.StateAndEventResponse>(
            HttpMethod.Post, route);
    }

    selectCell(gameId : number, cellId : number) : Promise<Model.StateAndEventResponse> {
    ...
    getEvents(gameId : number, query : Model.EventsQuery) : Promise<Model.Event[]> {
    ...
    createSnapshot(gameId : number, request : Model.CreateSnapshotRequest) : Promise<Model.SnapshotInfo> {
        const route = `/games/${gameId}/snapshots`;
        return ApiClientCore.sendRequest<Model.CreateSnapshotRequest, Model.SnapshotInfo>(
            HttpMethod.Post, route, request);
    }

    deleteSnapshot(gameId : number, snapshotId : number) : Promise<{}> {
        const route = `/games/${gameId}/snapshots/${snapshotId}`;
        return ApiClientCore.sendRequest<{}, {}>(
            HttpMethod.Delete, route);
    }

    getSnapshotsForGame(gameId : number) : Promise<Model.SnapshotInfo[]> {
        const route = `/games/${gameId}/snapshots`;
        return ApiClientCore.sendRequest<{}, Model.SnapshotInfo[]>(
            HttpMethod.Get, route);
    }

    loadSnapshot(gameId : number, snapshotId : number) : Promise<{}> {
